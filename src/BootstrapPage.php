...
class BootstrapPage {
    ...
    private $leftNav = null;   // string|Renderable|callable
    private $main = null;      // required
    private $rightNav = null;  // string|Renderable|callable
    ...
    public function setColumnWidths(int $left, int $main, int $right): self {
        $this->columnWidths = compact('left', 'main', 'right');
        $this->validateWidths();
        return $this;
    }
...
<body>
    ...
    private function renderHolyGrail(string $bp): void {
        $w = $this->columnWidths;
        $mainClass = "col-12 col-{$bp}-{$w['main']}";
        
        // If sidebars exist, add responsive classes
        $leftClass = "col-{$bp}-{$w['left']} d-none d-{$bp}-block";
        $rightClass = "col-{$bp}-{$w['right']} d-none d-{$bp}-block";
        ?>
